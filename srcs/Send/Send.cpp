...
static bool	isClose(const Connection &c) {
	...
	if (n == 400 || n == 401 || n == 403 || n == 404 || n == 408 || n == 429 ||  n == 500 || n == 501 || n == 502 || n == 503 || n == 504) {
		return true;
	}
	return false;
}
//curl --resolve ykusano.com:4242:127.0.0.1 http://ykusano.com:4242/
...
int	Server::_ConnectionClose(int fd, Connection &c) {
	std::cout << "Connection close [" << fd << "]" << std::endl;
	close(fd);
	c.closeCgiReadFd();
	c.closeCgiWriteFd();
	c.killPid();
